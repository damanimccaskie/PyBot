...
async def on_message(message):
    ...
    "corona", "paul", "walcot", "lowe", "hilary"]

    if message.author == client.user: # dont check for bad words of bot (can lead to infinite loop)
      return

   
    def find(msg, bad): #linear search
        for i in range(0, len(msg)):
            if msg[i:i+len(bad)] == bad and (i - 1 < 0 or msg[i-1] == ' ') and (i+len(bad) >= len(msg) or msg[i+len(bad)] == ' '):
                return True
        return False
